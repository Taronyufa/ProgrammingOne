...
node* makeOperation(node* a, node* b){
    ...
    if(getVar(a) == getVar(b)){
        ...
        if(areEquivalent(makeOperation(getThen(a), getThen(b)), makeOperation(getElse(a), getElse(b)))){
            return makeOperation(getElse(a), getElse(b));
        }else{
            node* temp = new node;
            temp = makeNode(getVar(a), makeOperation(getElse(a), getElse(b)), makeOperation(getThen(a), getThen(b)));
            return temp;
        }
    }
    ...
    if(getVar(a) < getVar(b)){
        node* temp = new node;
        temp = makeNode(getVar(a), makeOperation(getThen(a), b), makeOperation(getElse(a), b));
        return temp;
    }else{
        node* temp = new node; 
        temp = makeNode(getVar(b), makeOperation(getThen(b), a), makeOperation(getElse(b), a));
        return temp;
    }
}
